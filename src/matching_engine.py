...
class KartaMatcher(MatchEngine):
    ...
    def __init__(self, logger, disas):
        """Create the complete Basic Ctor for the matching context.
    ...
    ## this function has been modified to not print the state. Instead, it produces the JSON I want.
    def debugPrintState(self, error=False):
        """Print a detailed debugging trace of the matching state of each source file, including overall statistics.
        ...
        if log:
            self.logger.info("Matched Functions: %d/%d(/%d) (%d/%d)",## TODO Crucial Statistic
        ...
        for match_file in filter(lambda x: x.valid, self._match_files):
            ...
            filled_ref_files += (1 if not missed_ref_func else 0) ## can be used to determine if matched successfully. TODO TODO: what determines if a file matched successfully (what is the confidence threshold)
        #if log: 
        self.logger.info("File Statistics: Located %d/%d files, Finished %d/%d (%d/%d) files",
                                located_files, num_files, filled_files, num_files, filled_ref_files, num_ref_files)
        self.logger.info("------------------------------------------------------------------------")
        ##source total has duplication of data. I duplicate this just in case different models of the same version's records are mixed. A model made manually vs with Karta analyze vs. one made with a different analysis backend (ghidra, radare) could have a different number of functions matched
        
        if log:
            printed_ghost = False
            # Source map, by files
            for match_file in filter(lambda x: x.valid, self._match_files):##interesting
        ...
        return {
            ...
            "function":
            {
                "matches":map(lambda x: self.src_functions_ctx[x].name, self.function_matches.keys()),
                "srcTotal":num_src_functions,
                "actTotal":num_act_functions,
                "extMatched":len(filter(lambda x: x.matched(), self._src_external_functions.values())),
                "extTotal":num_ext_functions
            },
            "file":
            {
                 "matches":located_files,
                 "total":num_files,
                 "filled":filled_files,
                 "ref":
                 {
                  "filled":filled_ref_files, 
                  "total":num_ref_files
                 }
            }
        }
    ...
    ## seems like this data is/can be aggregated when finding the best match? matching uses several different techniques for hints, and each technique appears to have its own round
    ## maybe here would be a this could be a good place for matching?
    def roundMatchResults(self):
        """Declare the winners of the match round, and prepares for the next round.
        ...
        for match_record in self._match_round_candidates:
            ...
            self.logger.debug("Round match attempt: %s (%d) vs %s (0x%x): %f (+%f = %f)",
                                    src_candidate.name, src_index, bin_candidate.name, bin_candidate.ea, match_record['score'] - match_record['boost'], match_record['boost'], match_record['score'])
        ...
        if self._last_matching_step:
            ...
            for src_candidate in matching_couples:
                ...
                self.declareMatch(src_candidate.index, bin_candidate.ea, REASON_TRAPPED_COUPLE)## is this useful data?
                self._last_matching_step = False
                declared_match = True
                # store them for a later filter
                matched_src_index.add(src_candidate.index)## what later filter? these aren't the good ones?
    ...
    def recordRoundMatchAttempt(self, src_index, func_ea, score_boost, score, reason):
        """Record a match attempt into the round's records.
    ...
    ## Very, very promising. Seems to return a very useful value? I need to understand what they mean by probability. Is it probability of matching, or a measure of how close the libraries are to each other. Probably the latter
    def matchAttempt(self, src_index, func_ea, file_match=None):
        ...
        if file_match is not None and neighbour_match and src_index in [file_match._src_index_start, file_match._src_index_end] and \
            ...
            score_boost += getNeighbourScore()
        ## This is where the confidence score is calculated.
        # triple the bonus if both apply
        if score_boost >= 2 * getNeighbourScore():
            score_boost += getNeighbourScore()
        self.logger.debug("%s (%d) vs %s (0x%x): %f (+%f = %f)",
                            src_candidate.name, src_index, bin_candidate.name, bin_candidate.ea, score, score_boost, score + score_boost)
    ...
    def matchFiles(self):
        """Match the files, one matching round after the other."""
        ...
        try:
            ...
            self.debugPrintState(error=True)
            return
        #### This is where the magic ~~happens~~ becomes evident
    ...
    def prepareGUIEntries(self):
        """Prepare the entries list, according to the sorting logic we want in the GUI.
    ...
    def generateSuggestedNames(self):
        """Generate the suggested names for the binary functions."""
    ...
    def renameChosenFunctions(self, bin_eas, suggested_names):
        """Rename the chosen set of binary functions.
        ...
        for bin_ea in bin_eas:
            ...
            if bin_ea not in suggested_names:
                self.logger.warning("Failed to rename function at 0x%x, has no name for it", bin_ea) ## TODO this is effectively 0-confidence
                continue
            # rename it
